    ...
    >
      {TABS.map((tab) => (
        <NavLink
          ...
          className={({ isActive }) =>
            `flex-1 flex flex-col items-center justify-center gap-1 py-2.5 text-[10px] font-medium tracking-wide transition-colors ${
        ...
        >
          {({ isActive }) => (
            <>
              ...
              <span className={isActive ? 'text-indigo-400' : 'text-gray-600'}>{tab.label}</span>
            </>
          )}
        </NavLink>
      ))}
