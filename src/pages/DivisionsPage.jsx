...
function RankingsScreen({ div, sprintId, sprintName, myUserId, myDivId, onOpenPicker, onBack, totalPlayers, isGwLocked }) {
  ...
  const lastPromo = promLP !== null ? rows.filter(r => r.total_league_points >= promLP).length - 1 : -1
  const firstRel  = relLP  !== null ? rows.findIndex(r => r.total_league_points <= relLP) : -1
        ...
        {!loading && rows.length > 0 && (
          <div className="pb-32">
            {rows.map((row, i) => {
              ...
              const isPromo  = promLP !== null && row.total_league_points >= promLP && !div.is_highest
              const isRel    = relLP  !== null && row.total_league_points <= relLP
              const showRelDiv   = firstRel === i && i > 0
              const showPromDiv  = lastPromo === i && i < rows.length - 1 && !div.is_highest
